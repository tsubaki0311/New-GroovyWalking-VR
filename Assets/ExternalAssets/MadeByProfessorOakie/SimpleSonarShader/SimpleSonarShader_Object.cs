...
public class SimpleSonarShader_Object : MonoBehaviour
{
    ...
    // Queue of speeds of each ring
    private static Queue<float> speedQueue = new Queue<float>(QueueSize);

    private static Queue<float> widthQueue = new Queue<float>(QueueSize);
    ...
    [Header("Sonar Shader Settings")]
    [SerializeField] float sonarRingIntensity = 1.0f;
    [SerializeField] float sonarRingSpeed = 1.0f;
    [SerializeField] float sonarRingWidth = 10f;
    ...
    private void Start()
    {
        ...
        if(NeedToInitQueues)
        {
            ...
            for (int i = 0; i < QueueSize; i++)
            {
                ...
                speedQueue.Enqueue(0.0f);
                widthQueue.Enqueue(0.0f);
            }
        }
    ...
    [ContextMenu("StartSonarRingAtOrigin")]
    public void StartSonarRingAtOrigin()
    {
        StartSonarRing(Camera.current.transform.root.position, sonarRingIntensity, sonarRingSpeed, sonarRingWidth);
    }

    /// <summary>
    /// Starts a sonar ring from this position with the given intensity.
    /// </summary>
    public void StartSonarRing(Vector4 position, float intensity, float speed, float width)
    {
    ...
    private void SendSonarData()
    {
        ...
        foreach (Renderer r in ObjectRenderers)
        {
            ...
            r.material.SetFloatArray("_Speed", speedQueue.ToArray());
            r.material.SetFloatArray("_Width", widthQueue.ToArray());
        }
    }
